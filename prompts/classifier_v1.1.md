...
changed_from_previous: Circul8 reclassified from NOT_VENTURE_SCALE negative example to BORDERLINE teaching case (live run confirmed patent claim in record); lifestyle_hardware_shop pattern clarified to note that patents on small-throughput bespoke hardware do not substitute for scalable IP defensibility.
---
...
- `lifestyle_hardware_shop`: Bespoke fabrication, sub-$5M revenue plateau, no scaling intent. **Important:** patents on small-throughput hardware in niche markets (e.g., 100–300 bpd recovery units for spent drilling fluids) do NOT substitute for scalable IP defensibility — this pattern is about operational profile (bespoke fabrication, niche addressable market, no institutional equity trajectory) not the mere absence of patents. A patent on bespoke equipment + niche market + services-operator framing = BORDERLINE, not VENTURE_SCALE.
...
**BORDERLINE teaching case:**

7. **Circul8 Energy & Environment** (Presenting Company, 2024): The record states "have patented and commercialized the Delta series of 100/200 and 300 bpd recovery systems" for spent oil-based mud (OBM) management. The patent claim is real and is a HIGH-weight positive signal — this prevents a NOT_VENTURE_SCALE classification. However, the operational profile (100–300 bpd throughput, niche OBM disposal market, "principals who have spent their careers in the oil and gas space designing and operating cost effective recovery and end of life solutions" framing without institutional equity backing) matches the `lifestyle_hardware_shop` false-positive pattern. Patents on small-throughput bespoke hardware in niche markets without a scalable revenue trajectory do not substitute for venture-scale IP defensibility. → BORDERLINE, MEDIUM confidence, score ~5.5. This example demonstrates that VENTURE_SCALE requires both defensible IP AND a scalable business model — patents alone don't suffice when the underlying operation is bespoke fabrication for a niche market.
